...
First we will read in shapefiles from the GISdata folder in your R project directory.  We will use tmap to create basic maps of these spatial layers. Read more about tmap here: 
...
As a preliminary step, lets plot some of the wolfyht using the base plot function, and using some of the structure of the wolfyht object. Lets remind ourselves of the structure of the sf object wolfyht. 
```{r message=FALSE}
class(wolfyht)
...
moose_plot2
```
Notice that the highest habitat quality for moose tends to be in valley bottoms
...
sheep_plot2
```
High quality sheep habitat is slightly higher in elevation than moose habitat
...
## Always make your bounding box much larger than you think it needs to be bcause resampling can create NAs and bias around edges if the spatial extents of layers aren't the same!!!

ext(mask.raster) <- c(xmin=443680.6, xmax=650430.4, ymin=5618416, ymax=5789236) 	


#spatial resolution == GRAIN SIZE  != SCALE
#resample() starts at one corner of extent and uses moving window to resample all layers to same grain size and spatial extent (registraiton)

#set the resolution to 300 m 
res(mask.raster)<-300

#match projection to elc_habitat shapefile- can also take EPSG
...
plot(mask.raster)
#Value is NA outside of defined extent

#we could extract HSI values of points form polygons to build RSF, but we need to rasterize to make pretty maps
```
...
#rasterize uses extent, resolution, etc. of input layer. Can be mask or any existing raster layer.
...
#Interacts with ggplot well and good for plotting random forest models
#deer_w <- st_as_stars(elc_habitat, name = attr(elc_habitat, "DEER_W"))
#rasterizes faster than terra
...
#plot result
plot(wolf_w) #This is from before wolves were in Banff
plot(wolf_w_stars)
```
...
```{r, eval = FALSE}
#write raster layers to file (terra)
#use this when you don't want to redo your raster operations every time.
writeRaster(deer_w, here::here("Lab2","Output", "DEER_W.tiff"), overwrite = TRUE)
```

For today's lab we will us elevation and distance to human access rasters we created before the lab and re-load them from our working directory. 
```{r}
...
ext(goat_w)
ext(elevation2)# already the same
ext(disthighhumanaccess) #different from the others
```
...
#bilinear interpolation resamples linearly in 2 dimensions. Appropriate for continuous variables but not categorical variables
```

How can we zoom into just one area of a raster object? There are many reasons we might want to, and many ways. One easy way is to use `mapview()`. Note - mapview is not evaluated here because the html image does not play well with .md files - try evaluating within your own project
...
## Part 1c - Distance to Raster (Human Access Layer)
...
#default is minimum euclidean distance to nearest something

#there are other ways of calcualting distance: path, elevational, etc.

#use distance instead of density because you can always get density from distance_to
...
#renaming levels to clean up spelling
...
#create indicator variable for high or not high human access (Reclassification)
...
humanaccess_plot
```
Now you would go through and rasterize this following the steps above to make this a raster file. Note I already did this with the distance to high human access layer.

# Objective 2 - Home Range Analysis 

Note I've done these here for the individual level I added the pack-level analysis below. But our goal will be to calculate 99% minimum convex polygon for individual wolves in the Red Deer wolf pack.  Our first step is to change the wolfyht `sf` data.frame to a adehabitat::spatial points object. 
...
```{r warning=FALSE}
rd.data<-wolfyht[wolfyht$Pack=="Red Deer",] #subset one wolf pack
x<-rd.data$EASTING #define coordinate fields
y<-rd.data$NORTHING
xy<-cbind(x,y)#bind coordinate fields together
class(xy)
rd <- data.frame(as.character(rd.data$NAME)) #pull out names of wolves
coordinates(rd) <- xy # define xy as coordinates in sp dataframe 
...
names(rd)<-"NAME" #create new vector of names
rd<-rd[rd@data$NAME!="69" & rd@data$NAME!="81" & rd@data$NAME!="82" & rd@data$NAME!="84",] #subset out wolves that are not 69, 82 and 84
...
#note differences between individual-level homeranges and pack-level homeranges

#check area in square meters for each Red Deer wolf pack
as.data.frame(cp.rd)

#calculate area for different percents of MPC in square meters. 
mcp.area(rd, percent=seq(50, 100, by=5)) #5% steps from 50 to 100%

#how many locations do I need to estimate homerange size in an unbiased way?

#MCP is problematic because it's not statistical, has no mathematical basis, and extent of polygon is basicaly defined by telemetry locations. Can still be useful for unmarked animals from cameras.
```
...
image(red.deerUD) 
#we can do math on these and relate them to probability of use

#get polygons for home ranges (95 percentile KDE)
...
## Set up graphical parameters for the output of getvolumeUD  (isopleths)
...
#calculate 95% KDE for both wolf packs
...
#Utilization Distribution is a true probability density function

#Problem: you have to calculate UDs for each pack or each individual and then rescale them and add them together, because they're all scaled by the sample size of the data. 
```
...
#eventually we'll compare the used and available points using logistic regression assumming used and available distributions are both iid which of course isn't true because one is animal-generated and one is randomly generated

#This is a within-home range scale analysis

#assumes there are no boundaries to movement within homerange

#every model of availability has problems
...
It's very useful to look at patterns of use in isolation of patterns of availability before looking at avaialability distribution!

This is a 3rd order resource selection analysis (within-homerange)

4th order is diet scale
